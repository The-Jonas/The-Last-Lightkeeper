...
const char* kFragmentSrc =
    ...
    "        /* Onde ha CARIMBO manda o carimbo, sozinho. O cenario continua a\n"
    "           ganhar cor por estar no cone ou por ter luz; um personagem ou um\n"
    "           item carimbado, nao: o irmao esta no APICE do seu proprio cone,\n"
    "           por isso `vis` vale 1 nele e o `max` devolvia-lhe a cor toda\n"
    "           mesmo as escuras. Para o que e carimbado so conta a LUZ que lhe\n"
    "           chega. */\n"
    "        float sceneColor = max(vis, sceneLight * uLightColor);\n"
    "        float keepColor = clamp(mix(sceneColor, charLight, isChar), 0.0, 1.0);\n"
    "\n"
